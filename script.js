...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Número do WhatsApp para onde o pedido será enviado. Deve incluir DDI e DDD, sem espaços ou símbolos.
  const whatsappNumber = '5511982544973';

  // Objeto que armazena os itens adicionados ao carrinho. A chave é o nome do item,
  // e o valor contém o preço (string, com vírgula) e a quantidade selecionada.
  const cart = {};

  /**
   * Adiciona um item ao carrinho. Se o item já existir, incrementa sua quantidade.
   * @param {string} itemName Nome do item
   * @param {string} itemPrice Preço do item (string com vírgula)
   */
  ...
  /**
   * Atualiza a quantidade de um item no carrinho. Se a quantidade chegar a zero ou menor,
   * o item é removido do carrinho.
   * @param {string} itemName Nome do item
   * @param {number} delta Variação na quantidade (positivo ou negativo)
   */
  ...
  /**
   * Renderiza o conteúdo do carrinho na página. Calcula o total e exibe cada item
   * com botões para aumentar ou diminuir quantidade.
   */
  function renderCart() {
    const cartSection = document.getElementById('cart-section');
    const cartItemsDiv = document.getElementById('cart-items');
    const cartTotalDiv = document.getElementById('cart-total');
    if (!cartSection || !cartItemsDiv || !cartTotalDiv) return;
  ...
  function renderMenu() {
    ...
    function createCategoryBlock(catName) {
      ...
      category.items.forEach(item => {
        ...
        // Botão para adicionar este item ao carrinho
  ...
  renderMenu();
  // Inicializa a visualização do carrinho (ficará oculto se não houver itens)
  renderCart();
  ...
  orderForm.addEventListener('submit', event => {
    ...
    // Usa o carrinho para compilar os itens selecionados e calcular o valor total
    const names = Object.keys(cart);
    if (names.length === 0) {
    ...
    names.forEach(name => {
      ...
      orderLines.push(`${name}: ${qty} x R$ ${price} = R$ ${itemTotalStr}`);
    });
    ...
    // Usa o número de WhatsApp definido globalmente para construir a URL
